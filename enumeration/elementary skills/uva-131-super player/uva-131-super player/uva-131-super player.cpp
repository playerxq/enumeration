...
void Traverse()
{
    ...
    if(is_straight_flush())    {res = 0; return;}
    if(is_four())              {res = 1; return;}
    if(is_full())              {res = res < 2 ? res : 2; return;}
    if(is_flush())             {res = res < 3 ? res : 3; return;}
    if(is_straight())          {res = res < 4 ? res : 4; return;}
    if(is_three())             {res = res < 5 ? res : 5; return;}
    if(is_two())               {res = res < 6 ? res : 6; return;}
    if(is_one())               {res = res < 7 ? res : 7; return;}
}
...
*/
    ...
    while(cin>>poke[0])
    {
        for(int i=2; i<=10; ++i)
            cin>>poke[i-1];
        res = 8;
        for(int i=0; i<(1<<5); ++i)//从0到11111所有状态  为1则换相应的牌
        {
            int num = 0;
            memset_1(temp, poke);
            for(int j=0; j<5; ++j)
            ...
            for(int j=0; j<5; ++j)
				if(i&(1<<j)) 
					strcpy(temp[j], temp[(sequence[cnt++]+4)]);//第j位为1  将牌换至j位上 按序从牌堆取牌 cnt为牌堆用掉的牌数
            Traverse();
            if(res == 0) break;
        }
        cout<<"Hand: ";
